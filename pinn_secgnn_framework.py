...
class SEEEstimator(nn.Module):
    ...
    def forward(self, predictions: Dict, system_state: Dict) -> torch.Tensor:
        ...
        # SEE = R_secrecy / Energy (单位: bps/Hz/W)
        see = secrecy_rate / (energy + 1e-6)

        # ✅ 添加详细调试信息
        logger.debug(f"SEE computation: R_sec={secrecy_rate.mean():.4f} bps/Hz, "
                     f"P_total={energy.mean():.1f}W, "
                     f"SEE={see.mean():.6f} bps/Hz/W")

        # 合理性检查
        if torch.any(see < 1e-6):
            logger.warning(f"Very low SEE: {see.mean():.6f} bps/Hz/W")

        if torch.any(see > 1.0):
            logger.warning(f"High SEE: {see.mean():.4f} bps/Hz/W")

        return see

    def _compute_secrecy_rate(self, predictions, system_state):
        """
        计算保密速率（完全修正版 - 增强功率和信道处理）
        """
        try:
            # 提取信道
            ...
            # ✅ 关键修复1：合理的功率归一化
            max_power_dbm = 46.0  # 46 dBm
            max_power_watts = 10 ** ((max_power_dbm - 30) / 10)  # ≈ 39.81 W

            W_power = torch.sum(torch.abs(W) ** 2, dim=(1, 2), keepdim=True)  # [batch, 1, 1]

            # 避免除以0
            W_power = torch.clamp(W_power, min=1e-10)

            # 归一化
            W_normalized = W * torch.sqrt(max_power_watts / W_power)

            # ✅ 验证归一化结果
            final_power = torch.sum(torch.abs(W_normalized) ** 2, dim=(1, 2))
            if torch.any(final_power < 1.0) or torch.any(final_power > 100.0):
                logger.warning(f"Beamforming power after normalization: "
                               f"mean={final_power.mean().item():.2f}W, "
                               f"min={final_power.min().item():.2f}W, "
                               f"max={final_power.max().item():.2f}W")
            ...
            if noise_power < 1e-15:
                noise_power = 1e-13
                logger.warning(f"Noise power too low, setting to {noise_power}")
            ...
            for k in range(K):
                ...
                # ✅ 关键修复3：检查信道增益
                channel_power = torch.mean(torch.abs(h_eff) ** 2)
                if channel_power < 1e-8:
                    logger.warning(f"User {k} effective channel power very low: {channel_power:.2e}")
                ...
                # 限制SINR范围避免数值溢出
                sinr = torch.clamp(sinr, min=1e-6, max=1e8)

                R_user_k = torch.log2(1 + sinr)

                # ✅ 关键修复5：检查速率合理性
                if torch.any(R_user_k < 0.01):
                    logger.warning(f"User {k} rate very low: {R_user_k.mean().item():.4f} bps/Hz, "
                                   f"signal={signal.mean().item():.2e}, "
                                   f"interference={interference.mean().item():.2e}, "
                                   f"SINR={sinr.mean().item():.2f}")
            ...
            for e in range(E):
                ...
                for k in range(K):
                    ...
                    sinr_eve = signal_eve / (interference_eve + noise_power + 1e-10)
                    sinr_eve = torch.clamp(sinr_eve, min=1e-6, max=1e8)
                    R_eve_k = torch.log2(1 + sinr_eve)
            ...
            # ✅ 关键修复6：详细的警告信息
            if torch.any(secrecy_rate < 0.1):
                logger.warning(
                    f"Low secrecy rate! R_user: {R_user.mean():.4f}, "
                    f"R_eve: {R_eve_max.mean():.4f}, "
                    f"R_sec: {secrecy_rate.mean():.4f} bps/Hz")

            if torch.all(secrecy_rate == 0):
                logger.error(
                    f"Zero secrecy rate detected! R_user: {R_user.mean():.4f}, "
                    f"R_eve: {R_eve_max.mean():.4f}")
    ...
    def _compute_energy(self, predictions, system_state):
        ...
        try:
            ...
            # ✅ 修正：合理的功率范围映射
            # 假设最大发射功率为 46dBm = 39.81W
            max_tx_power_dbm = 46.0
            max_tx_power = 10 ** ((max_tx_power_dbm - 30) / 10)  # ≈ 39.81W

            # 计算归一化功率
            W_power_normalized = torch.sum(W[..., 0] ** 2 + W[..., 1] ** 2, dim=(-2, -1))

            # 映射到实际功率
            # 假设网络输出范围经过tanh后在[-1, 1]，平方后[0, 1]
            # 映射到[0, max_tx_power]
            tx_power = W_power_normalized * max_tx_power / 4.0  # 平均约10-20W

            # 确保在合理范围
            tx_power = torch.clamp(tx_power, min=5.0, max=max_tx_power)
            ...
            # ✅ 修正：速度应该已经是实际值（通过tanh * max_velocity得到）
            # trajectory输出范围：[-max_velocity, +max_velocity]
            # 直接使用绝对值
            v_h_actual = torch.abs(v_horizontal)  # 实际水平速度
            ...
            # 诱导功率（考虑速度影响）
            V_0 = 7.2  # 平均旋翼速度 (m/s)
            term = torch.sqrt(1 + v_h_actual ** 4 / (4 * V_0 ** 4)) - v_h_actual ** 2 / (2 * V_0 ** 2)
            term = torch.clamp(term, min=0.0)  # 确保非负
            P_induced_actual = P_induced * torch.sqrt(term + 1e-6)

            # 垂直功率
            # W = mg ≈ 20 N, P_vertical = W * v_z
            P_vertical = 20.0 * v_vertical  # 约0-20W

            # UAV总功耗
            uav_power = P_blade_actual + P_induced_actual + P_vertical

            # 确保UAV功耗在合理范围（悬停约188W，最大约250W）
            uav_power = torch.clamp(uav_power, min=150.0, max=250.0)

            # ========== 3. RIS控制功率 ==========
            # RIS每个元素约0.1W控制功率
            num_ris_elements = system_state.get('num_ris_elements', 64)
            ris_power_per_element = 0.1  # W
            ris_power = torch.full((batch_size,),
                                   num_ris_elements * ris_power_per_element,
                                   device=tx_power.device)  # 约6.4W
            ...
            # ✅ 添加调试信息
            logger.debug(f"Power breakdown: TX={tx_power.mean():.1f}W, "
                         f"UAV={uav_power.mean():.1f}W, "
                         f"RIS={ris_power.mean():.1f}W, "
                         f"Total={total_energy.mean():.1f}W")
            ...
            # 返回默认值
            batch_size = predictions['ris_phases'].size(0)
            return torch.full((batch_size,), 224.4, device=predictions['ris_phases'].device)
