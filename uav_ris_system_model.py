...
class SystemParameters:
    ...
    path_loss_exponent_bs_ris_los: float = 2.0
    path_loss_exponent_bs_ris_nlos: float = 3.5
    path_loss_exponent_ris_user_los: float = 2.0
    path_loss_exponent_ris_user_nlos: float = 3.0
    path_loss_exponent_ris_eve_los: float = 2.2
    path_loss_exponent_ris_eve_nlos: float = 3.5
    ...
    # -------------------- 新增：传输功率参数 --------------------
    transmit_power_dbm: float = 30.0  # dBm, BS发射功率
    ...
    def __post_init__(self):
        ...
        self.transmit_power = 10 ** ((self.transmit_power_dbm - 30) / 10)
        k_B = self.boltzmann_constant
        T = self.temperature_kelvin
        B = self.bandwidth
        NF_linear = 10 ** (self.noise_figure_db / 10)
        self.noise_power = k_B * T * B * NF_linear
